import { useState, createContext, useEffect, useContext, SyntheticEvent } from 'react';
...
export const WorkspaceContext = createContext<{workspace: Workspace, setWorkspace: any}>({workspace: initialWorkspace, setWorkspace: () => {}});
...
const resizerStyles = { backgroundColor: '#2c3645', width: '10px', height: '100%', cursor: 'ew-resize', position: 'absolute' as 'absolute'};
...
function App() {
  ...
  const resizeLeft = (event: SyntheticEvent<Element, Event>, {node, size, handle}: ResizeCallbackData) => {
  ...
  const resizeRight = (event: SyntheticEvent<Element, Event>, {node, size, handle}: ResizeCallbackData) => {
      ...
      <Container>
        ...
        <ContentRow>
          ...
          <ResizableBox 
            ...
            handle={<div 
                style={{ left: width.fileTree + width.editor - 10 + 'px', ...resizerStyles }}
              />}
          >
            <Editor width={width.editor - 10} />
          </ResizableBox>
          <PDFPane
            style={{ width: width.viewer }}>
            <PDF />
          </PDFPane>
        </ContentRow>
      </Container>
